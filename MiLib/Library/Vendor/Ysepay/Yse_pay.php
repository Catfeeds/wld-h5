...
class Yse_pay
{
    ...
    function get_money($parr)
    {
        ...
        $biz_content_arr = array(
            "merchant_usercode"=>$parr['merchant_usercode'],
        );
    ...
    function get_inner($parr){
        ...
        $biz_content_arr = array(
            ...
            "effect_date" => $parr['effect_date'],
            "expire_date" => $parr['expire_date'],
            "bank_account_type" =>$parr['bank_account_type'],
            "bank_card_type" => $parr['bank_card_type'],
            "bank_name" => $parr['bank_name'],
            "bank_account_no" => $parr['bank_account_no'],
            "bank_account_name" => $parr['bank_account_name'],
            "bank_city" => $parr['bank_city'],
            "bank_telephone_no" => $parr['bank_telephone_no'],
            "cert_type" => $parr['cert_type'],
            "cert_no" => $parr['cert_no'],
            "cert_expire"=>$parr['cert_expire'],
            "month_num_limit"=>$parr['month_num_limit'],
            "month_amount_limit"=>$parr['month_amount_limit']
        );
    ...
    function weixin_pay($parr){
        $myParams = array();
        $myParams['charset'] = 'utf-8';
        $myParams['method'] = "ysepay.online.jsapi.pay";      // 接口名
        $myParams['partner_id'] = "wld17375717292";   // 商户号
        $myParams['sign_type'] = 'RSA';
        $myParams['timestamp'] = date('Y-m-d H:i:s', time());
        $myParams['version'] = '3.0';
        $myParams['notify_url'] = $parr['notify_url'];  //yspay.ngrok.cc/pay/respond_notify.php';
        $biz_content_arr = array(
            ...
            "business_code" => "2010002",//"01000009",
            "currency" => "CNY",
            "sub_openid" =>$parr['sub_openid'], // 微信用户所关注商家公众号openid
            "shopdata" => date('Ymd'),
        );
        $myParams['biz_content'] = json_encode($biz_content_arr);//构造字符串
        ksort($myParams);
     ...
     * 平台内代付（单笔）
     */
    function get_inner_df($parr)
    {
        $myParams = array();
        $myParams['charset'] = 'utf-8';
        $myParams['method'] = 'ysepay.df.single.quick.inner.accept';
        $myParams['partner_id'] = 'wld17375717292';  //发起方
        $myParams['sign_type'] = 'RSA';
        $myParams['timestamp'] = date('Y-m-d H:i:s', time());
        $myParams['version'] = '3.0';
        $myParams['notify_url'] =$parr['notify_url'];
        ...
        $biz_content = array(
            ...
            "total_amount" => $parr['total_amount'],
            "subject" => $parr['subject'],
            "payee_user_code" => $parr['payee_user_code'],  //收款方
            "payee_cust_name" => $parr['payee_cust_name'],
        );
    ...
    function send_money_to_card($parr){
        $myParams = array();
        $myParams['charset'] = 'utf-8';
        $myParams['method'] = 'ysepay.merchant.withdraw.quick.accept';
        $myParams['partner_id'] = $parr['partner_id'];//'wld17375717292';
        $myParams['sign_type'] = 'RSA';
        $myParams['timestamp'] = date('Y-m-d H:i:s', time());
        $myParams['version'] = '3.0';
        $myParams['notify_url'] = $parr['notify_url'];
    ...
    function curl_money_to_card($qa)
    {
        $ch = curl_init("https://commonapi.ysepay.com/gateway.do");
        ...
        if (curl_errno($ch)) {
            ...
            $sign = $response['sign'];
            //echo $sign;
            $data = json_encode($response['ysepay_merchant_withdraw_quick_accept_response'], JSON_UNESCAPED_UNICODE);
    ...
    /** curl 获取 https 请求 商户余额查询
     * @param qa 要发送的数据
     */
    /** curl 获取 https 请求 余额查询
     * @param qa 要发送的数据
     */
    function curl_query_money($qa)
    {
        $ch = curl_init("https://commonapi.ysepay.com/gateway.do");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($qa));
        ...
        if (curl_errno($ch)) {
            ...
            return $response;
            $sign = $response['sign'];
            $data = json_encode($response['ysepay_merchant_balance_query_response'], JSON_UNESCAPED_UNICODE);
            //var_dump($data);
            /* 验证签名 仅作基础验证*/
            if ($this->sign_check($sign, $data) == true) {
                echo "验证签名成功!";
     ...
     * 通用线上订单查询
     */
    function searchOnlineOrder($parr){
        $myParams = array();
        $myParams['charset'] = 'utf-8';
    ...
    /** curl 获取 https 请求 线上订单查询
     * @param qa 要发送的数据
     */
    function curl_query_order($qa)
    {
        $ch = curl_init("https://search.ysepay.com/gateway.do");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($qa));
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
    ...
    function curl_https_scan($qa)
    {
        ...
        if (curl_errno($ch)) {
            ...
            return $response;
            $sign = $response['sign'];
            $data = json_encode($response['ysepay_online_qrcodepay_response'], JSON_UNESCAPED_UNICODE);
            $data = $this->arrayToString($data);
            /* 验证签名 仅作基础验证*/
            if ($this->sign_check($sign, $data) == true) {
                return $response; //echo "验证签名成功!";
    ...
    /** curl 获取 https 请求 单笔代收加急
     * @param qa 要发送的数据
     */
    function curl_https_ds($qa)
    {
        $ch = curl_init("https://ds.ysepay.com/gateway.do");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($qa));
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
    ...
    /** curl 获取 https 请求 单笔代收订单查询
     * @param qa 要发送的数据
     */
    function curl_https_ds_search($qa)
    {
        $ch = curl_init("https://searchds.ysepay.com/gateway.do");
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, http_build_query($qa));
        curl_setopt($ch, CURLOPT_FOLLOWLOCATION, 1);
        curl_setopt($ch, CURLOPT_HEADER, 0);
    ...
    function ds_bill($parr){
        $myParams = array();
        $myParams['charset'] = 'utf-8';
        $myParams['method'] = 'ysepay.ds.bill.downloadurl.get';
        $myParams['partner_id'] = 'wld17375717292';
        $myParams['sign_type'] = 'RSA';
        $myParams['timestamp'] = date('Y-m-d H:i:s', time());
        $myParams['version'] = '3.0';
