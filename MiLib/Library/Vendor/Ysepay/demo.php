...
//        $this->param['pfxpassword'] = "821321";
        $this->param = array();
        $this->param['pfxpath'] = 'http://' . $_SERVER['HTTP_HOST'] . "/MiLib/Library/Vendor/Ysepay/certs/shanghu_test.pfx";
        $this->param['businessgatecerpath'] = 'http://' . $_SERVER['HTTP_HOST'] . "/MiLib/Library/Vendor/Ysepay/certs/businessgatetest.cer";
    ...
    function get_code($order)
    {
        ...
        $myParams['seller_id'] = 'shanghu_test';//'wld17375717292';
    ...
    function get_dfjj($order)
    {
        ...
        $myParams['notify_url'] = 'https://testapi.iweilingdi.com/api.php/Pay/Test/respond_notify';//'http://yspay.ngrok.cc/pay/respond_notify.php';
        ...
        $biz_content_arr = array(
            ...
            "total_amount" => "1.00",
            "subject" => "测试",
            "bank_name" => "建设银行华兴支行",//"建设银行潇湘支行",//"建设银行恒达支行",//"望城坡支行",//
            "bank_city" => "长沙市",
            "bank_account_no" => "6217002920123595952",//"6217002920125089541",//"6232512920277176",//"6217002920132384174",//
            "bank_account_name" => "童向",//"李泰然",
            "bank_account_type" => "personal",// corporate :对公账户; personal :对私账户
            "bank_card_type" => "debit"    //  debit:借记卡;credit:信用卡 unit:单位结算卡
        );
        $myParams['biz_content'] = json_encode($biz_content_arr, JSON_UNESCAPED_UNICODE);//构造字符串
        // var_dump($myParams);
        ksort($myParams);
    ...
    function app_pay($order){
        ...
        $biz_content_arr = array(
            ...
            "seller_id" => "wld17375717292", // 收款方银盛支付用户名
            "seller_name" => "长沙微领地网络科技有限公司", // 授权方银盛支付客户名
            "timeout_express" => "96h",  // 超时时间
            "business_code" => "3010002",
            "currency" => "CNY",
            "bank_type" => "1902000", // 1902000 微信  1903000 支付宝
        );
    ...
    function alipay_H5($order){
        ...
        $myParams['timestamp'] = date('Y-m-d H:i:s', time());
        $myParams['version'] = '3.0';
        $myParams['notify_url'] = 'https://testapi.iweilingdi.com/api.php/Pay/Test/respond_notify';  //yspay.ngrok.cc/pay/respond_notify.php';
        $myParams['return_url'] = 'https://testapi.iweilingdi.com/api.php/Pay/Test/return_notify';//'https://testapi.iweilingdi.com/api.php/Pay/Test/return_alipay'; // 跳转页面
        $biz_content_arr = array(
            "out_trade_no" => "$order", // 订单号
            "subject" => "商品", // 产品名
            "total_amount" => "0.01", //总金额
            "seller_id" => "shanghu_test", // 收款方银盛支付用户名
            "seller_name" => "银盛支付商户测试公司", // 授权方银盛支付客户名
            "timeout_express" => "96h",  // 超时时间
            "business_code" => "01000009",
            "currency" => "CNY",
            "bank_type" => "1903000", // 1902000 微信  1903000 支付宝
        );
        $myParams['biz_content'] = json_encode($biz_content_arr);//构造字符串
        ksort($myParams);
        $data = $myParams;
        ...
        dump($myParams);
        $action = "https://openapi.ysepay.com/gateway.do";
//        $action = "https://qrcode.ysepay.com/gateway.do";
        $def_url = "<br /><form style='text-align:center;' method=post action='" . $action . "' target='_blank'>";
        while ($param = each($myParams)) {
            $def_url .= "<input type = 'hidden' id='" . $param['key'] . "' name='" . $param['key'] . "' value='" . $param['value'] . "' />";
        }
    ...
    */
    ...
    function searchOrder(){
        ...
        $myParams['version'] = '3.0';
        $biz_content_arr = array(
            "out_trade_no" => "20170906101447"//$parr['out_trade_no'], // 订单号
        );
        $myParams['biz_content'] = json_encode($biz_content_arr);//构造字符串
        ksort($myParams);
        $signStr = "";
...
//        $ch = curl_init("https://df.ysepay.com/gateway.do");
        ...
        if (curl_errno($ch)) {
           ...
           // $data = json_encode($response['ysepay_df_single_quick_accept_response'], JSON_UNESCAPED_UNICODE);
            $data = json_encode($response['ysepay.df.single.query_response'], JSON_UNESCAPED_UNICODE);
            $data = $this->arrayToString($data);
            // dump($data);
            /* 验证签名 仅作基础验证*/
            if ($this->sign_check($sign, $data) == true) {
    ...
    /** curl 获取 https 请求 h5微信支付,支付宝支付接口
     * @param qa 要发送的数据
     */
    function curl_https_pay($qa)
    {
        $ch = curl_init("https://openapi.ysepay.com/gateway.do");
        //$ch = curl_init("https://mertest.ysepay.com/openapi_gateway/gateway.do");
        curl_setopt($ch, CURLOPT_POST, 1);
