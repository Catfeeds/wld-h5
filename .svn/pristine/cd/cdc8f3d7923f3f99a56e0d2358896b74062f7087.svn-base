...
abstract class Driver {
     ...
     * @return void
     */
    public function startTrans($fenku='') {
        $this->initConnect(true);
        if ( !$this->_linkID ) return false;

        if ($fenku == 1) {
        	$this->_linkID->prepare('set autocommit=0');
        	$this->_linkID->prepare('set xa=on');
        }
    ...
    public function insert($data,$options=array(),$replace=false) {
        ...
        $tstable = str_replace('`','', $this->parseTable($options['table']));
        $shardingconf = C('DB_SHARDING');
        $shardingarr = explode(',', $shardingconf);
        if (in_array($tstable, $shardingarr) && count($shardingarr) > 0) {
            $fields[]   =  'c_id';
            $values[]   =  'next value for MYCATSEQ_'.$tstable;
        }
        foreach ($data as $key=>$val){
            if(is_array($val) && 'exp' == $val[0]){
