...
class Wxpay_client_pub extends Common_util_pub {
     ...
     */
    function createXml() {
        $this->parameters["appid"] = C('APPID'); //公众账号ID
        $this->parameters["mch_id"] = C('MCHID'); //商户号
...
class UnifiedOrder_pub extends Wxpay_client_pub {
    ...
    function createXml() {
        try {
            ...
            $this->parameters["appid"] = C('APPID'); //公众账号ID
            $this->parameters["mch_id"] = C('MCHID'); //商户号
            $this->parameters["spbill_create_ip"] = '119.39.71.95'; //终端ip	    
            $this->parameters["nonce_str"] = $this->createNoncestr(); //随机字符串
            $this->parameters["sign"] = $this->getSign($this->parameters); //签名
            return $this->arrayToXml($this->parameters);
...
class NativeCall_pub extends Wxpay_server_pub {
    ...
    function createXml() {
        if ($this->returnParameters["return_code"] == "SUCCESS") {
            $this->returnParameters["appid"] = C('APPID'); //公众账号ID
            $this->returnParameters["mch_id"] = C('MCHID'); //商户号
