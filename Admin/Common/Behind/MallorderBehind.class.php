...
class MallorderBehind {
    ...
    function sheetIndexnt(){
        ...
        if (!empty($pnickname)) {
            $w[] = "pu.c_nickname LIKE '%".$pnickname."%'";array('like', "%{$pnickname}%");
        }
        $name = trim(I('name'));//供货商
        if (!empty($name)) {
            $w[] =  "s.c_name LIKE '%".$name."%'";
        }
        ...
        $sql="select o.*,s.c_name,adr.*,u.c_nickname,u.c_phone,pu.c_nickname as pname from t_supplier_order as o left join t_supplier as s on s.c_ucode=o.c_acode left join t_users as u on u.c_ucode=o.c_ucode left join t_users as pu on pu.c_ucode=o.c_scode left join t_supplier_order_address as adr on adr.c_orderid=o.c_orderid $w1 ORDER BY o.c_id desc LIMIT $s,$conud";
        ...
        foreach ($data as $k1 => $v1){
            ...
            $data[$k1]['goods'] = M('supplier_order_details')->where($wh)->select();
        } 

        $date[0][0]=array("订单号","订单状态","购买人名称","购买人注册手机","商家名称","供货商名称","商品名称","商品类型","商品属性","商品单价(×数量)","总金额","邮费","商品状态","收货人","联系方式","收货地址","下单时间","支付时间","订单附言","活动名称");
        $k=1;
        foreach ($data as $k=>$v) {
            $k++;
    ...
    private function getExcel($fileName,$data){
        ...
        foreach ($data as $ke=>$row){
            foreach($row as $key=>$rows){
             ...
             $objPHPExcel->getActiveSheet()->mergeCells("A{$column}:U{$column}");//合并单元格
             $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getFont()->setSize(12);//字体
             $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getFont()->setBold(true);//粗体
             //背景色填充
             $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID);
             $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getFill()->getStartColor()->setARGB('FFB8CCE4');
                }else{
                    if(!empty($rows)){
                        array_unshift($rows,$key+1);
                        $objPHPExcel->getActiveSheet()->setSharedStyle($sharedStyle1,"A{$column}:U{$column}");//设置边框
                    }
                }
                if($rows['1']=='订单号'){
                    //背景色填充
                    $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getFill()->setFillType(\PHPExcel_Style_Fill::FILL_SOLID);
                    $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getFill()->getStartColor()->setARGB('FF4F81BD');
                }
                $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getAlignment()->setVertical(\PHPExcel_Style_Alignment::VERTICAL_CENTER);//垂直居中
                $objPHPExcel->getActiveSheet()->getStyle("A{$column}:U{$column}")->getAlignment()->setWrapText(true);//换行
                //行写入
                $span = ord("A");
