...
class ScanpayScanpay {
    ...
    function CreateScanpayOrder($parr) {
        if (!empty($parr['ucode'])) {
            //查询开户商户可用金额
            $mparr['ucode'] = $parr['ucode'];
            $result = IGD('Balance','User')->GetSyncYesMoney($mparr);
            if ($result['code'] == 0 && $result['data']['c_ysdrmoney'] < $parr['balance'] && $parr['balance'] > 0) {
                return Message(3001,'可抵扣金额不足');
            }
        }

        //风控交易状态
        $ufparr['ucode'] = $parr['acode'];
        $ufparr['sign'] = 3;
        ...
        //风控单笔交易部分
        $ruleparr['ucode'] = $parr['acode'];
        $result = IGD('Funds','Info')->GetFundsRule($ruleparr);
        $fundsrule = $result['data'];
        if ($parr['money'] > $fundsrule['c_spenoney'] && $fundsrule['c_spenoney'] > 0) {
            return Message(3005,'单笔交易金额不能超过'.$fundsrule['c_spenoney']);
        }     
        ...
        if ($countinfo['c_money'] > $fundsrule['c_sdaymoney'] && $fundsrule['c_sdaymoney'] > 0) {
            return Message(3006,'单日交易金额不能超过'.$fundsrule['c_sdaymoney']);
        }
        ...
        if (!empty($parr['open_id'])) {
            $scandata['c_headimg'] = $parr['open_id'];
        }     
        ...
        if (is_weixin()) {
            $openidtj = $this->FindOpenidLock($scandata['c_unionid']);
        } else {
    ...
    function BalanceOrder($ncode, $sign) {
        ...
        if ($sign == 1) {
            ...
            if (!empty($orderinfo['c_unionid'])) {
                $openidtj = $this->FindOpenidLock($orderinfo['c_unionid'],1);
            } else {
            ...
            if ($shopprofit > 0) {
                ...
                $arr['ucode'] = $orderinfo['c_acode'];  //操作用户编码
                $arr['scode'] = $orderinfo['c_acode'];  //商家编码    
                $arr['bcode'] = $orderinfo['c_ucode'];  //用户编码
                $arr['orderid'] = CreateOrder('f');
                $arr['key'] = $orderinfo['c_ncode'];
                $arr['desc'] = '平台扫码余额支付订单交易资金操作';
                $arr['total_money'] = $orderinfo['c_total_price'];
                $arr['money'] = $opmoney;
                $arr['source'] = 9; // 1普城订单,2后台,3活动,4蜜城订单,5普城跨界,6提现,7注册,8老注册,9扫码,10转发,11绑定,12跨界扫码,13普城购返,14普城推返,15蜜城跨界,16普通退款,17蜜城退款,18红包',
            ...
            $msgdata['sendnum'] = 1;
            $msgdata['title'] = '系统消息';
            $msgdata['content'] = '收到金额' . floatval($orderinfo['c_money']) . '元，来自小蜜扫码支付';
            $msgdata['tag'] = 2;
            ...
            if (!empty($orderinfo['c_cashierid'])) {     //给收银员推送消息
                ...
                if ($cashinfo) {
                    ...
                    $msgdata['sendnum'] = 1;
                    $msgdata['title'] = '系统消息';
                    $msgdata['content'] = '收款金额' . floatval($orderinfo['c_money']) . '元，来自小蜜扫码支付';
                    $msgdata['tag'] = 2;
    ...
    function PayOrder($parr) {
        ...
        $scw['c_ncode'] = $orderid;
        $orderinfo = M('Scanpay')->where($scw)->find();
        ...
        if($shopprofit > 0){
            ...
            if ($payrule == 1) {
                $paycontent = '收到金额' . floatval($orderinfo['c_money'])  . '元，来自支付宝扫码支付';
            } else if ($payrule == 2 || $payrule == 3) {
                $paycontent = '收到金额' . floatval($orderinfo['c_money'])  . '元，来自微信扫码支付';
            } else {
                $paycontent = '收到金额' . floatval($orderinfo['c_money'])  . '元，来自小蜜扫码支付';
            }
    ...
    function CheckRebate($orderinfo, $ucodetj, $openidtj) {
        ...
        //写入平台利润记录
        $profitparr['orderid'] = $orderinfo['c_ncode'];
    ...
    //查询微信openid是否被锁定
    function FindOpenidLock($openid,$type) {
        if (empty($openid)) {
            return false;
        }
        if (is_weixin() || $type == 1) {
            $result = M('Scanpay_tuijian')->where(array('c_unionid' => $openid))->order('c_id asc')->find();
        } else {
            $result = M('Scanpay_tuijian')->where(array('c_openid' => $openid))->order('c_id asc')->find();
        }
    ...
    function AddOpenidLock($openid, $acode,$unionid,$type) {
        ...
        if (!empty($unionid)) {
            $unionwh['c_unionid'] = $unionid;
            $tjinfo = M('Scanpay_tuijian')->where($unionwh)->find();
            if ($tjinfo) {
    ...
    function BindRegister($parr) {
        ...
        if (!empty($acode)) {
            ...
            if (!$tuijiandata) {
                if (is_weixin()) {
                    $pdacode = $this->FindOpenidLock($parr['unionid']);
                    if ($pdacode) {
                        $result = $this->AddUcodeLock($ucode, $pdacode['c_pcode'],$parr['unionid']);
                    } else {
                        $result = $this->AddUcodeLock($ucode, $acode,$parr['unionid']);
                    }
                } else {
