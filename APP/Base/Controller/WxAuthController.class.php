 ...
 * 不需要登录的基类
 */
class WxAuthController extends AuthController {
    ...
    public function GetOpenid()
    {
        ...
        if (!empty($_GET['code'])) {
            ...
            $url = $this->createOauthUrlForCode($redirectUrl,'snsapi_userinfo');
    ...
    public function GetAlOpenid()
    {
        if (!session('alipay_authinfo')) {
            ...
            $url = $config['alipay_auth_url'].'?auth_type=auth_user&returnurl='.$returnurl;
            header("Location:$url");die;
        }
